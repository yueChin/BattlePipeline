...
namespace ET
{
    ...
    public class TimerComponent: Entity
    {
        ...
        public async ETTask<bool> InternalWaitTillAsync(long tillTime, ETCancellationToken cancellationToken)
        {
        ...
        public async ETTask<bool> InternalWaitFrameAsync(ETCancellationToken cancellationToken)
        {
            return await InternalWaitAsync(1, cancellationToken);
        }

        //不是很懂异步的，别用这种接口，用了，也要传入CancelToken
        public async ETTask<bool> InternalWaitAsync(long time, ETCancellationToken cancellationToken)
        {
            if (time == 0)
            {
                return true;
