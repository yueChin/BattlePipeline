...
namespace ET
{
    ...
    public class CoroutineLockComponentAwakeSystem: AwakeSystem<CoroutineLockComponent>
    {
        public override void Awake(CoroutineLockComponent self)
        {
            ...
            {
                self.list.Add(EntityFactory.CreateWithParentAndId<CoroutineLockQueueType>(self, ++self.idGenerator));
            }
        }
    }
    ...
    public static class CoroutineLockComponentSystem
    {
        ...
        public static async ETTask<CoroutineLock> Wait(this CoroutineLockComponent self, CoroutineLockType coroutineLockType, long key, int time = 60000)
        {
            ...
            {
                coroutineLockQueueType.Add(key, EntityFactory.CreateWithParentAndId<CoroutineLockQueue>(self, ++self.idGenerator, true));
        ...
        {
            CoroutineLock coroutineLock = EntityFactory.CreateWithParentAndId<CoroutineLock, CoroutineLockType, long, int>(self, ++self.idGenerator, coroutineLockType, key, count, true);
            if (time > 0)
            {
